...
/*Helper functions to find container*/
struct weighted_container* find_container(struct task_struct * p,struct list_head * this)
{
	int count =0;
	struct list_head *r,*qq;
	struct weighted_container *tmp;
  list_for_each_safe(r,qq,this)
  {	      
	      tmp = list_entry(r,struct weighted_container,container_list);
	      
	       if(tmp->cid == p->cid)
		{ 
			if(!(--tmp->total_processes))list_del(r);
			return tmp;
		}
	    if(count == *(p->array->nr_active_cwrr_q))return NULL;
...
void fastcall sched_exit(task_t * p)
{
	unsigned long flags;
	runqueue_t *rq;	
	int del_c = 0,found_in_q = 0;
   
	struct weighted_container *tmp = NULL; 
	...
	{   /*as usual loop through both queues to find the container*/
 		 read_lock(&rwsem);	 
	     tmp = find_container(p,p->array->cwrr_q);	
		 found_in_q = 1;
	     if(!tmp)
		 {
			found_in_q = 0;
			tmp = find_container(p,p->array->cwrr_sq);		
		 }
	     read_unlock(&rwsem);
		/*no more processes remaining in container. mark for deletion
			and remove from queue*/
		if(!(tmp->total_processes)){del_c = 1;}
...
{   
  int retval,found_to_q = 0,found_from_q = 0,found_to_sq = 0,found_from_sq = 0;
  int oldprio, oldpolicy = -1;int dele =0,dele2 = 0;
  prio_array_t *array;
  unsigned long flags;
  runqueue_t *rq;
  struct weighted_container *to= NULL,*from = NULL;
 ...
 recheck:
  ...
  if(policy == SCHED_CWRR)/*regardless of current policy*/
    ...
    if(p->policy == SCHED_CWRR && p->cid != param->sched_priority)
	{
	  ...
	  read_lock(&rwsem);
      from = find_container(p,p->array->cwrr_q);found_from_q = 1;
	  if(!from)
	  {
		  found_from_q = 0;
		  from = find_container(p,p->array->cwrr_sq);
		  if(from!=NULL)found_from_sq = 1;
	  }
	  ...
	  spin_lock(&(from->container_lock));
	  if(!from->total_processes){dele = 1;}
	  else if(found_from_q == 1 && from->active_flag == 0)
		{
		  write_lock(&rwsem);
		  dequeue_container(p,p->array->cwrr_q);
		  enqueue_container(p,p->array->cwrr_sq);
	  ...
	  spin_unlock(&(from->container_lock));
	 
	  /*find container to add to*/
	 read_lock(&rwsem);
      to = find_container(p,p->array->cwrr_q);found_to_q = 1;
	  if(!to)
	  {
		  found_to_q = 0;
		  to = find_container(p,p->array->cwrr_sq);
		  if(to!=NULL)found_to_sq = 1;
	  }
	  if(!found_to_q && !found_to_sq){task_rq_unlock(rq, &flags);read_unlock(&rwsem);return -EFAULT; }
	  read_unlock(&rwsem);
	  
	  spin_lock(&(to->container_lock));
	  to->total_processes++;
	  p->parent_container = to;
	  p->cid = to->cid;
	  
	  spin_unlock(&(to->container_lock));
	  p->time_slice = 100;
  ...
  else if(p->policy == SCHED_CWRR && policy != SCHED_CWRR)/*change from CWRR to any other*/
    {
      ...
      if(!--p->parent_container->total_processes)
	  {
		...
		read_lock(&rwsem);
      to = find_container(p,p->array->cwrr_q);dele2 = 1;
	  if(!to)
	  {
		  dele2 = 0;
		  to = find_container(p,p->array->cwrr_sq);
	  ...
	  if(!dele2){task_rq_unlock(rq, &flags);read_unlock(&rwsem);return -EFAULT; }
	  read_unlock(&rwsem);
	  }
